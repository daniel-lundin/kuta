...
feature("file watch", scenario => {
  scenario(
    "trigger new run on file change",
    ({ after, given, when, then, and }) => {
      let kutaEmitter;
      ...
      when("test run completes", () => kutaEmitter.waitForCompletedRun());

      and("a file change is triggered", () =>
        spawn("touch", ["tests/fixtures/passing-tests.js"])
      );

      then("tests should run again", () => kutaEmitter.waitForCompletedRun());
    }
  );

  scenario(
    "should not exit on failures in watch mode",
  ...
  scenario(
    ...
    ({ given, when, and, then }) => {
      ...
      when("runner is completed", () => kutaEmitter.waitForCompletedRun());

      and("a file change is triggered", () =>
        spawn("touch", ["tests/fixtures/slow-tests.js"])
      );

      when("tests are restarting", () => kutaEmitter.waitForOutput());
