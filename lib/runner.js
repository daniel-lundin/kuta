...
// function resultHasFailures(testResults) {
//   return utils.summarizeResults(testResults).errors !== 0;
// }

function executeInProcessPool(files, matches, requires, processCount, reporterName, timeout, bailMode, logger) {
  const processes = startProcessPool(processCount, requires);

  const testResults = [];
  const reporter = reporters.create(reporterName, logger, files.length, processCount);
  ...
  function onPartialTestResults(message) {
    ...
    // if (bailMode && resultHasFailures(message)) {
    //   console.log('killing it early');
...
function run(files, matches, requires, processCount, reporter, timeout, bailMode, logger) {
  logger.hideCursor();
  return executeInProcessPool(files, matches, requires, processCount, reporter, timeout, bailMode, logger)
    .then((results) => {
      logger.showCursor();
      return results;
    })
