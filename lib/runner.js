...
function executeInProcessPool(files, match, requires, processCount, reporterName, timeout, logger) {
  const opts = {
    cwd: process.cwd()
  };
  ...
  function onSuiteError(message, process, resolve) {
    logger.log('suiteError', message.testFile, message.stack);
    if (!startSuiteInProcess(process)) {
      process.kill();
      resolve();
    }
...
function run(files, match, requires, processCount, reporter, timeout, logger) {
  logger.hideCursor();
  return executeInProcessPool(files, match, requires, processCount, reporter, timeout, logger)
    .then((results) => {
      logger.showCursor();
      return results;
    })
