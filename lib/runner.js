...
function isGroupEmpty(group) {
  const groupSummary = summarizeResults(group);
...
function handleTestResults(message, log) {
  const summarizedResults = summarizeResults(message.results);
...
function startProcessPool(files, match, requires, processCount, timeout, log) {
  const opts = {
    cwd: process.cwd()
  };
...
function run(files, match, requires, processCount, timeout, log=logger.log) {
  return startProcessPool(files, match, requires, processCount, timeout, log)
    .then((results) =>
