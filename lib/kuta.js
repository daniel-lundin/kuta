...
function createTestGroup(groupDescription = null) {
  ...
  test._runTests = (testFile, match, timeout, outerEaches = createEachHook()) => {
    const startTime = new Date();
    const beforePromises = arrayAsPromise(befores);
    const eachHooks = outerEaches.extend(beforeEaches, afterEaches);
    ...
    return testChain
      .then(() => test._runGroups(testFile, match, timeout, eachHooks))
