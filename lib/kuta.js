...
function createTestGroup(groupDescription = null) {
  ...
  test._runGroups = (testFile, match, timeout, outerBeforeEaches = [], outerAfterEaches = []) => {
    return groups.reduce((promise, group) => {
      return promise.then(() => {
        ...
        return group._runTests(
          ...
          childMatch,
          timeout,
          outerBeforeEaches.concat(beforeEaches),
          outerAfterEaches.concat(afterEaches)
        );
      });
    }, Promise.resolve());
  };
  ...
  test._runTests = (testFile, match, timeout, outerBeforeEaches = [], outerAfterEaches = []) => {
    const startTime = new Date();
    ...
    const testChain = tests.reduce((promise, currentTest, testIndex) => {
      ...
      return promise.then(() => {
        return arrayAsPromise(outerBeforeEaches.concat(beforeEaches))
          ...
          .then(() => arrayAsPromise(outerAfterEaches.concat(afterEaches)));
      });
    }, beforePromises);

    return testChain
      .then(() => test._runGroups(testFile, match, timeout, outerBeforeEaches, outerAfterEaches))
