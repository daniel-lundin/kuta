...
function createTestGroup(groupDescription = null) {
  ...
  function sendPartialResult(IPC, testFile) {
    const results = test._results(testFile);
  ...
  test._runTests = (testFile, match, timeout, IPC, outerHooks = createHookContext()) => {
    const startTime = new Date();
...
function runTests(testFile, match, timeout, IPC) {
  ...
  return test._runTests(testFile, match, timeout, IPC)
