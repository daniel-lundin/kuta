...
function extractFailingTests(group, parentGroup = null) {
  const errors = group.results
    .filter(test => test.result === common.TEST_FAILURE)
    .map(test => Object.assign({}, test, { parentGroup }));
  return errors.concat(
    ...group.groups.map((g) => extractFailingTests(g, group))
  );
}
